...
export class Actors {
  ...
  createActorTypeField;
  createActorCreateField;
  sheetLocator;
  tabCharacteristics;
  tabGear;
  tabTalents;
  ...
  tabWeapons;
  tabCrew;
  constructor(page, actorName, actorType) {
    this.page = page;
    ...
    this.sheetLocator = this.page.locator(".sheet", { has: this.page.locator(`text=${this.actorName}`) });
  ...
  async closeSheet() {
    const closeButton = this.page
      .locator(".sheet", { has: this.page.locator(`text=${this.actorName}`) })
      .locator(".close");
  ...
  async checkStat(statName, statValue) {
    if (statName === "armor") {
    ...
    if (statName === "encumbranceMax") {
      ...
      await expect(
        this.sheetLocator.locator(`input[name="data.stats.${statName.replace("Max", "")}.max"]`)
      ).toHaveValue(statValue);
      ...
      await expect(this.sheetLocator.locator(`input[name="data.stats.${statName.replace("s", "c")}"]`)).toHaveValue(
        statValue
      );
    } else if (["Brawn", "Agility", "Intellect", "Cunning", "Willpower", "Presence"].includes(statName)) {
      await expect(this.sheetLocator.locator(`input[name="data.characteristics.${statName}.value"]`)).toHaveValue(
        statValue
      );
    } else if (["armour"].includes(statName)) {
      await expect(this.sheetLocator.locator(`input[name="data.stats.${statName}.value"]`)).toHaveValue(statValue);
    } else if (statName === "customizationHardPoints") {
      await expect(this.sheetLocator.locator(`input[name="data.stats.customizationHardPoints.value"]`)).toHaveValue(
        statValue
      );
    }
  }
  async equipItem(itemName) {
    await this.sheetLocator
      .locator(".item", { has: this.page.locator(`text=${itemName}`) })
      .locator(".toggle-equipped")
      .click();
  }
  async editItem(itemName) {
    await this.sheetLocator
      .locator(".item", { has: this.page.locator(`text=${itemName}`) })
      .locator(".item-edit")
      .click();
  }
  async checkSkillModifiers(skillName, modifierName, modifierValue) {
...
export class Items {
  ...
  tabLongDesc;
  upgradeName;
  constructor(page, itemName, itemType) {
    this.page = page;
    ...
    this.sheetLocator = this.page.locator(".sheet", { has: this.page.locator(`input[value="${this.itemName}"]`) });
  ...
  async setStat(statName, statValue) {
    ...
    if (["defence", "soak", "encumbrance", "hardpoints", "rarity"].includes(statName)) {
      ...
      ["Wounds", "Strain", "Brawn", "Agility", "Intellect", "Cunning", "Willpower", "Presence"].includes(statName)
    ) {
  ...
  async addDirectModifier(modifierType, modifier, modifierValue) {
  ...
  async addTalentModifier(talentNumber, modifierType, modifier, modifierValue) {
  ...
  async learnTalent(talentNumber) {
