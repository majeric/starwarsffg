...
import { HomesteadUpgradeData } from "./item/homesteadupgrade-data.js";
...
export function registerDataModels(): void {
  ...
  for (const [type, model] of Object.entries(ITEM_DATA_MODELS)) {
    // FIXME(types): fvtt-types rejects item TypeDataModel subclasses here even
    // though Foundry's registry accepts them at runtime.
    CONFIG.Item.dataModels[type] = model as unknown as (typeof CONFIG.Item.dataModels)[string];
  }
}
