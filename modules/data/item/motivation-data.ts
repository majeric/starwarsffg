...
/**
 * DataModel for the `motivation` item type. It composes the shared core and
 * basic item fields plus the motivation category.
 *
 * Phase 5 is schema-only (ADR-010): no prepare hooks here, so legacy ItemFFG
 * preparation runs unchanged.
 */
export class MotivationData extends BaseItemData {
  static defineSchema(): foundry.data.fields.DataSchema {
    const { StringField } = foundry.data.fields;
    return {
      ...core(),
      ...basic(),
      type: new StringField({ initial: "ambition" }),
    };
  }
}
