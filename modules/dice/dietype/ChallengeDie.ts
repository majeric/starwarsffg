export class ChallengeDie extends foundry.dice.terms.DiceTerm {
  ...
  /** @override */
  get formula(): string {
    return `${this.number}${(this.constructor as typeof ChallengeDie).DENOMINATION}${this.modifiers.join("")}`;
  }
  ...
  async roll(options?: any): Promise<any> {
    ...
    (roll as any).ffg = CONFIG.FFG.CHALLENGE_RESULTS[(roll as any).result];
    return roll;
  }
